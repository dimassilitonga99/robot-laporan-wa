...
// ── WAKTU & SAPAAN ────────────────────────────────────
function getWaktu() {
  var j = new Date(Date.now() + 8*3600000).getUTCHours();
  return j>=5&&j<11?'Pagi':j>=11&&j<15?'Siang':j>=15&&j<19?'Sore':'Malam';
}

function balasanSapaan(sender) {
  var nama  = getNama(sender);
  var waktu = getWaktu();
  var sapa  = nama
...
// Kata sapaan yang dideteksi
var KATA_SAPAAN = [
  'halo','hai','hi','hello','hey','hei','holla','ola','yo','sup','howdy',
  'pagi','siang','sore','malam',
  'selamat','met pagi','met siang','met sore','met malam',
  'assalamualaikum','assalamu','waalaikumsalam','waalaikum','salam',
  'permisi','maaf ganggu','excuse me','good morning','good afternoon','good evening','good night'
];

function isSapaan(low) {
  for (var i = 0; i < KATA_SAPAAN.length; i++) {
    var k = KATA_SAPAAN[i];
    if (low === k ||
        low.startsWith(k + ' ') ||
        low.startsWith(k + ',') ||
...
// Kata terima kasih
var KATA_TERIMAKASIH = [
  'terima kasih','terimakasih','makasih','thanks','thank you','thx','tq','ty',
  'tengkyu','tengkyuu','mksh','trims','trimakasih'
];

function isTerimakasih(low) {
  for (var i = 0; i < KATA_TERIMAKASIH.length; i++) {
    var k = KATA_TERIMAKASIH[i];
    if (low === k || low.startsWith(k + ' ') || low.startsWith(k + '!') || low.startsWith(k + ',')) return true;
  }
  return false;
}

function balasanTerimakasih(sender) {
  var nama = getNama(sender);
  var responses = [
    'Sama-sama' + (nama ? ', *' + nama + '*' : '') + '! \ud83d\ude0a\n\nJika ada yang ingin dicari lagi, ketik *menu* ya.',
    'Dengan senang hati' + (nama ? ', *' + nama + '*' : '') + '! \ud83d\ude0a\n\nSemoga bermanfaat! Ketik *menu* jika butuh bantuan lagi.',
    'Tentu' + (nama ? ', *' + nama + '*' : '') + '! Senang bisa membantu \ud83d\ude0a\n\nAda yang lain? Ketik *menu*.'
  ];
...
var NAMA_TOKO = {
  nk:'Nasional Kitchen', tdm:'Perabot Mama TDM',
  oesapa:'Perabot Mama Oesapa', kefa:'Perabot Mamaku Kefamenanu', cp:'Central Perabot'
};
var TOKO_LIST = [
  {kode:'nk',nama:'Nasional Kitchen'},{kode:'tdm',nama:'Perabot Mama TDM'},
  {kode:'oesapa',nama:'Perabot Mama Oesapa'},{kode:'kefa',nama:'Perabot Mamaku Kefamenanu'},
...
function loadExcel() {
  try {
    ...
    var wb=xlsx.readFile(EXCEL_PATH), ws=wb.Sheets[wb.SheetNames[0]];
    var rows=xlsx.utils.sheet_to_json(ws,{defval:0,range:1});
    DATA_BARANG=rows.map(function(r){
      var item={kode:String(r['Kode Item']||'').trim().toUpperCase(),nama:String(r['Nama Item']||'').trim().toUpperCase(),jenis:String(r['Jenis']||'').trim(),merek:String(r['Merek']||'').trim(),satuan:String(r['Satuan']||'').trim(),harga:{}};
      Object.keys(TOKO_COLS).forEach(function(k){var c=TOKO_COLS[k];item.harga[k]={ecer:parseFloat(r[c.ecer]||0)||0,ambil:parseFloat(r[c.ambil]||0)||0,stok:parseInt(r[c.stok]||0)||0};});
      return item;
...
function saveExcel() {
  try {
    var rows=DATA_BARANG.map(function(d){var row={'Kode Item':d.kode,'Nama Item':d.nama,'Jenis':d.jenis,'Merek':d.merek,'Satuan':d.satuan};Object.keys(TOKO_COLS).forEach(function(k){var c=TOKO_COLS[k];row[c.ecer]=d.harga[k].ecer;row[c.ambil]=d.harga[k].ambil;row[c.stok]=d.harga[k].stok;});return row;});
    var wb=xlsx.utils.book_new();xlsx.utils.book_append_sheet(wb,xlsx.utils.json_to_sheet(rows),'Data Barang');xlsx.writeFile(wb,EXCEL_PATH);return true;
...
function cariBarang(keyword) {
  ...
  var words=q.split(/\s+/);
  return DATA_BARANG.filter(function(d){return words.every(function(w){return d.nama.indexOf(w)>=0||d.kode.indexOf(w)>=0;});});
}

function formatRp(n){var v=parseFloat(n)||0;return v===0?'Rp -':'Rp '+v.toLocaleString('id-ID');}

function formatHasil(items, tokoKode, sender) {
  var namaToko=NAMA_TOKO[tokoKode]||'-';
  var nama=getNama(sender);
  var suffix = nama ? '\n\nSemoga membantu, *'+nama+'*! \ud83d\ude0a' : '\n\nSemoga membantu! \ud83d\ude0a';

  if(items.length===0) return '\u274c Barang tidak ditemukan.\n\nCoba kata kunci berbeda.\nContoh: _dandang eagle 20_';
  if(items.length===1){
    var d=items[0],h=d.harga[tokoKode];
  ...
  items.forEach(function(d,i){var h=d.harga[tokoKode];msg+=(i+1)+'. *'+d.nama+'*\n   \ud83d\udd16 '+d.kode+' | '+d.satuan+'\n   \ud83d\udcb0 Ecer: '+formatRp(h.ecer)+' | Ambil: '+formatRp(h.ambil)+'\n   \ud83d\udcca Stok: '+(h.stok>0?h.stok+' '+d.satuan:'Kosong')+'\n';if(i<items.length-1)msg+='\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\n';});
  return msg+'\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501'+suffix;
}

function updateStok(kode,tokoKode,jumlah){
  var k=kode.trim().toUpperCase();
  for(var i=0;i<DATA_BARANG.length;i++){if(DATA_BARANG[i].kode===k){DATA_BARANG[i].harga[tokoKode].stok=jumlah;saveExcel();return DATA_BARANG[i];}}
  return null;
}
...
function sapaanTim(nm){return 'Selamat '+getWaktu()+' Team '+nm;}
...
app.post('/webhook', async function(req,res){
  ...
  try {
    ...
    if(!sudahDisapa[sender]) {
      ...
      if(!['menu','halo','hai','hi','mulai','start'].includes(low)) return;
    }
    ...
    if(isAdmin(sender)){
      ...
      if(low.startsWith('namakontak ')){
        ...
        var nk=parts[0].replace(/[^0-9]/g,''), nv=parts.slice(1).join(' ');
        KONTAK[nk]=nv;saveKontak(KONTAK);
    ...
    if(low.startsWith('cari ')){
      ...
      if(s0.mode==='cari'&&s0.tokoKari){
        await kirimWA(sender,formatHasil(cariBarang(kw),s0.tokoKari,sender));
      } else {
    ...
    if(low.startsWith('stok ')){
      ...
      await kirimWA(sender,'\u2705 Stok diperbarui!\n\ud83c\udfe6 '+NAMA_TOKO[tkS]+'\n\ud83d\udd16 '+itmS.kode+'\n\ud83d\udce6 '+itmS.nama+'\n\ud83d\udcca Stok baru: *'+jmlS+' '+itmS.satuan+'*');return;
    }

    // ── RESET ──
    if(['0','batal','menu','mulai','start'].includes(low)){
    ...
    if(s.mode==='cari'&&!s.tokoKari){
      ...
      if(ci>=0&&ci<TOKO_LIST.length){
        ...
        if(s.pendingKw){
          await kirimWA(sender,formatHasil(cariBarang(s.pendingKw),s.tokoKari,sender));
          delete s.pendingKw;
    ...
    if(s.mode==='cari'&&s.tokoKari){
      if(!msg) return;
      await kirimWA(sender,formatHasil(cariBarang(msg),s.tokoKari,sender));
      setTimeout(async function(){await kirimWA(sender,'\ud83d\udd0d Cari lagi di *'+NAMA_TOKO[s.tokoKari]+'*?\nKetik nama/kode atau *0* kembali ke menu.');},800);
      return;
    }
    ...
    if(!s.menu){
      ...
      else if(msg==='4'){if(!isMember(sender)){await kirimWA(sender,'\ud83d\udeab *Akses Ditolak*\n\nFitur ini hanya untuk member terdaftar.');return;}sesi[sender]={mode:'cari'};await kirimWA(sender,MSG_PILIH_TOKO_CARI);}
    ...
    var nama3=s.menu===3?'Marketplace Perabot Mama':TOKO[s.toko],laporan='';
    if(image&&image.length>0){
      await kirimWA(sender,'Foto diterima, sedang diproses...');
      try{
        var pr=s.menu===1?'Baca data penjualan toko "'+nama3+'" tanggal '+tgl(s.kemarin)+'. Buat laporan lengkap WhatsApp dengan emoji, kassa, total, metode bayar, jenis penjualan. Format rupiah Rp X.XXX.XXX.':s.menu===2?'Baca data harga barang toko "'+nama3+'" tanggal '+tgl(s.kemarin)+'. Buat laporan WhatsApp dengan sapaan sesuai jam, barang baru/naik/turun harga.':'Baca data marketplace tanggal '+tgl(s.kemarin)+'. Buat laporan WhatsApp per toko, channel, metode bayar.';
      ...
      if(s.menu===1) laporan=lPenjualan(msg,nama3,s.kemarin);
      if(s.menu===2) laporan=lHarga(msg,nama3,s.kemarin);
    ...
    if(laporan){
      ...
      sesi[sender]={};
      var namaSelesai=getNama(sender);
      setTimeout(async function(){
        var pesanSelesai = namaSelesai
          ? 'Selesai! \ud83d\ude0a\n\nTerima kasih, *'+namaSelesai+'*!\nKirim *menu* untuk laporan berikutnya.'
          : 'Selesai! Kirim *menu* untuk laporan berikutnya.';
        await kirimWA(sender,pesanSelesai);
      },1000);
    }
